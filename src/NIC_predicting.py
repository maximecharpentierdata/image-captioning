...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(description="Compute predictions for all the pictures in the test set")

    parser.add_argument("--dest_path", help="Where to write the predictions file", default=PREDICTIONS_PATH)
    # parser.add_argument("--input_path", help="Where to read the test images features")
    parser.add_argument("--model_path", help="Path to the .h5 file containing model weights", default=MODEL_PATH)
    parser.add_argument("--force", help="Force recomputing if the file alreday exists", action="store_true")
    ...
    if os.exists(args.dest_path) and not args.force:
